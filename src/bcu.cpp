...
// SD Card pins (ESP32S3 devkit-c1)
#define SD_SCK   39
#define SD_MISO  40
#define SD_MOSI  41
#define SD_CS    38
...
int transmission_time = BMS_COMMUNICATE_TIME;
byte VmaxCell = (byte)(VMAX_CELL / 0.1);
byte VminCell = (byte)(VMIN_CELL / 0.1);
byte TempMaxCell = TEMP_MAX_CELL;
byte dVmax = (DVMAX / 0.1);
...
void debugBMUmsg(int Module) {
  ...
  for (short i = 0; i < CELL_NUM; i++) {
    Serial.print(BMU_Package[Module].V_CELL[i] * 0.02);
    Serial.print("V.  ");
  }
  Serial.println();

  Serial.print("TEMP[2]: ");
  Serial.print(BMU_Package[Module].TEMP_SENSE[0] * 0.6 + 2);
  Serial.print("C.  ");
  Serial.print(BMU_Package[Module].TEMP_SENSE[1] * 0.6 + 2);
  Serial.println("C.  ");
  Serial.println();
}
void debugBMUFault(int Module) {
  Serial.printf("OV_CRI: %X\n", BMU_Package[Module].OVERVOLTAGE_CRITICAL);
  Serial.printf("LV_CRI: %X\n", BMU_Package[Module].LOWVOLTAGE_CRITICAL);
  Serial.printf("OVT_CRI: %X\n", BMU_Package[Module].OVERTEMP_CRITICAL);
  Serial.printf("OVDIV_CRI: %X\n", BMU_Package[Module].OVERDIV_VOLTAGE_CRITICAL);
  Serial.println();
  Serial.printf("OV_W: %X\n", BMU_Package[Module].OVERVOLTAGE_WARNING);
  Serial.printf("LV_W: %X\n", BMU_Package[Module].LOWVOLTAGE_WARNING);
  Serial.printf("OVT_W: %X\n", BMU_Package[Module].OVERTEMP_WARNING);
  Serial.printf("OVDIV_W: %X\n", BMU_Package[Module].OVERDIV_VOLTAGE_WARNING);
  Serial.println();
}
void debugAMSstate() {
